...
(function() {
  var EOL, FILE_ENCODING, build_dir, changeDir, checkConfiguration, cleanUpDir, cleanUpDirWorker, coffee, coffeeSources, colors, compileAssets, compileCoffee, compileCoffeeDir, compileLess, compileLessDir, compiledAssets, configuration, copyLavaFiles, copyPluginFiles, copyPremiumFiles, copyToTestServer, createTmpDir, defaultLava, downQueue, exists, fs, fse, getAbsDir, getCoffeeFiles, lava, lavaVars, lavaVarsWorker, less, logging, namespaceClasses, namespaceClassesWorker, path, program, queue, upQueue, wrench, yaml, _allowed_extensions, _other_extensions,
  ...
  */
  ...
  _allowed_extensions = ["css", "html", "jpg", "js", "md", "php", "png", "po", "pot", "twig", "txt", "yml", "yaml"];

  _other_extensions = ["git", "gitattributes", "gitignore", "less", "coffee", "meta", "dir"];
  ...
  */
  ...
  exports.run = function(done) {
    ...
    program.version('1.0.0').option('-d, --distribute', 'Build distribution version').option('-v, --verbose', 'Prints debug stuff to screen').option('-n, --nonamespacing', 'Does not change lava_ into plugin namespace if this flag is present').option('-w, --wordpress', 'Excludes premium code for wordpress.org repository').parse(process.argv);
    program.parse(process.argv);
    return checkConfiguration();
  };

  checkConfiguration = function(run) {
    var i;
    if (run == null) {
      run = 0;
    }
    ...
    if (!fs.existsSync(getAbsDir('/src/plugin.yaml'))) {
  ...
  copyPluginFiles = function() {
    ...
    try {
      ...
      src = getAbsDir('/src/');
      dest = getAbsDir(build_dir);
      return fse.copy(src, dest, copyPremiumFiles);
    } catch (e) {
      logging.error('Error copying files to build directory');
      console.error(e);
      return process.exit();
  ...
  lavaVarsWorker = function(dir) {
    ...
    for (_i = 0, _len = files.length; _i < _len; _i++) {
      ...
      if (fs.statSync(absFile).isDirectory()) {
        ...
        if (file.substr(-4) === '.txt' || file.substr(-4) === '.php') {
          code = fs.readFileSync(absFile, FILE_ENCODING);
          for (variable in lava) {
